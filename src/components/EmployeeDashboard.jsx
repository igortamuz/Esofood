...
import {
  ...
  updateDoc,
  serverTimestamp,
} from "firebase/firestore"; // Mantém apenas o necessário para operações Firestore
import { signOut } from "firebase/auth"; // Mantém apenas o necessário para signOut
import { LogOut, CheckCircle, Clock } from "lucide-react";
import { QRCodeCanvas } from "qrcode.react";
import OrderList from "./OrderList";
import { db, auth } from "../firebase"; // Importa db e auth do arquivo centralizado
...
function EmployeeDashboard({ user }) {
  ...
  const handleCreateOrder = async (e) => {
    ...
    try {
      ...
      alert(
        "Falha ao criar o pedido. Verifique as regras do Firebase e a conexão."
      );
    }
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <p>
          Bem-vindo,{" "}
          <span className="font-bold text-yellow-300">{user.displayName}</span>!
        </p>
      ...
      <div className="card">
        ...
        >
          ...
          <select
            ...
            onChange={(e) =>
              setNewOrder({ ...newOrder, combo: e.target.value })
            }
            className="input-field"
          >
            {combos.map((combo) => (
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <OrderList
          ...
          actions={[
            {
              label: "Pronto",
              handler: (id) => updateOrderStatus(id, "ready"),
              color: "green",
            },
            {
              label: "Gerar QR",
              handler: (order) => setQrCodeOrder(order),
              color: "blue",
            },
          ]}
        />
        ...
        <div className="modal-overlay" onClick={() => setQrCodeOrder(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-2xl font-bold mb-2">
              Pedido de {qrCodeOrder.clientName}
            </h3>
